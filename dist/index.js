...
class Epub {
    constructor(options, output) {
        this.defer = (0, helpers_1.makePromise)();
        this.id = "";
        this.uuid = "";
        const self = this;
        if (output) {
        ...
        this.options = (0, helpers_1.extend)({
...
`;
        ...
        this.options.content = this.options.content.map(function (content, index) {
            ...
            // Fix Author Array
            if (content.author && (0, helpers_1.isString)(content.author)) {
                content.author = [content.author];
            }
            else if (!content.author || !(0, helpers_1.isArray)(content.author)) {
                content.author = [];
            }
            ...
            $("img").each(function (index, elem) {
                ...
                if (image) {
                    ...
                    id = (0, helpers_1.uuid)();
                    const { dir } = content, mediaType = mime.getType(url.replace(/\?.*/, ""));
    ...
    render() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield self.downloadAllImage();
            if (self.options.verbose) {
                console.log("Making Cover...");
            }
            yield self.makeCover();
            if (self.options.verbose) {
                console.log("Generating Epub Files...");
            }
            let result = yield self.genEpub();
            if (self.options.verbose) {
                console.log("Done.");
            }
            return result;
    ...
    generateTempFile() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!this.options.css) {
                this.options.css = fs.readFileSync(path.resolve(__dirname, "../templates/template.css"), "utf-8");
            }
...
`);
            ...
            fs.writeFileSync(path.resolve(self.uuid, "./OEBPS/content.opf"), yield ejs.renderFile(opfPath, self.options));
            ...
            fs.writeFileSync(path.resolve(self.uuid, "./OEBPS/toc.ncx"), yield ejs.renderFile(ncxTocPath, self.options));
            ...
            fs.writeFileSync(path.resolve(self.uuid, "./OEBPS/toc.xhtml"), yield ejs.renderFile(htmlTocPath, self.options));
        });
    }
    makeCover() {
        if (!this.options.cover) {
        ...
        const self = this, destPath = path.resolve(this.uuid, ("./OEBPS/cover." + this.options._coverExtension));
        return new Promise((resolve, reject) => {
            let writeStream;
            if (!self.options.cover)
                throw new Error('missing options.cover');
    ...
    downloadImage(options) {
        ...
        const self = this, filename = path.resolve(self.uuid, ("./OEBPS/images/" + options.id + "." + options.extension));
    ...
    genEpub() {
        ...
        const self = this, cwd = this.uuid;
        return new Promise((resolve, reject) => {
            let archive = archiver("zip", { zlib: { level: 9 } }), output = fs.createWriteStream(self.options.output);
            if (self.options.verbose) {
